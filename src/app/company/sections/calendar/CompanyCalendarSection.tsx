...
// Generate Mock Data scoped to a single company
const generateMockBookings = (): Booking[] => {
  ...
  for (let i = -30; i < 30; i++) {
    ...
    // Fewer bookings per day since it's just one company
...
const COMPANY_BOOKINGS = generateMockBookings();
...
export default function CompanyCalendarSection() {
  ...
  const filteredBookings = useMemo(() => {
    return COMPANY_BOOKINGS.filter(booking => {
  ...
  const stats = useMemo(() => {
    const dayBookings = COMPANY_BOOKINGS.filter(b => isSameDay(b.date, selectedDate));
    return [
  ...
  const upcomingDays = useMemo(() => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      const bookings = COMPANY_BOOKINGS.filter(b => isSameDay(b.date, date));
        ...
        <div className="space-y-1">
          {(() => {
            ...
            while (day <= endDate) {
              for (let i = 0; i < 7; i++) {
                const cloneDay = day;
                const hasBookings = COMPANY_BOOKINGS.some(b => isSameDay(b.date, cloneDay));
